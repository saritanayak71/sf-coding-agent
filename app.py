...
:root {
    ...
    --accent: #00d4ff; --text: #e2e8f0; --muted: #64748b; --success: #10b981;
}
...
if st.session_state.sf_connected:
    ...
    if st.button("Disconnect"):
        for k in ["sf_connected","sf_client","sobjects","schema","selected_object","generated","rag_ready","rag_engine"]:
            st.session_state[k] = False if k == "sf_connected" else ([] if k == "sobjects" else None)
        st.rerun()
else:
    ...
    st.caption("Credentials stored in session only — never written to disk or GitHub.")

    if st.button("Connect to Salesforce"):
        if not all([sf_username, sf_password, sf_token, sf_instance, sf_consumer_key, sf_consumer_secret]):
            st.error("All six fields are required.")
        else:
            with st.spinner("Connecting via OAuth..."):
                try:
...
if st.session_state.sf_connected:
    ...
    if st.session_state.rag_ready:
        ...
        uploaded_files = st.file_uploader(
            "Upload best practices docs (optional)",
            type=["txt", "md"],
...
if st.session_state.sf_connected and st.session_state.rag_ready:
    ...
    with col_left:
        requirement = st.text_area(
            "Requirement",
            placeholder="e.g. When an Opportunity is closed won, create a follow-up Task assigned to the owner due 3 days later.",
            height=120,
        )
    ...
    with col_right:
        ...
        if st.session_state.schema:
            field_count = len(st.session_state.schema.get("fields", []))
            st.caption(f"↳ {field_count} fields loaded from org")

    if st.session_state.schema:
        with st.expander(f"📋 {selected_obj} schema preview"):
            for f in st.session_state.schema.get("fields", [])[:20]:
...
if st.session_state.generated:
    ...
    with tab1:
        ...
        st.download_button(
            "Download Trigger",
            g.get("trigger", ""),
            file_name=f"{selected_obj}Trigger.trigger",
            mime="text/plain",
        )
    ...
    with tab2:
        ...
        st.download_button(
            "Download Handler",
            g.get("handler", ""),
            file_name=f"{selected_obj}TriggerHandler.cls",
            mime="text/plain",
        )
    ...
    with tab3:
        ...
        st.download_button(
            "Download Test Class",
            g.get("test_class", ""),
            file_name=f"{selected_obj}TriggerHandlerTest.cls",
            mime="text/plain",
        )
